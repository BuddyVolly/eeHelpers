...
def generate_grid(aoi, dx, dy, marginx=0, marginy=0):
    boundbox = aoi.geometry().bounds().buffer(distance=1,
    ...
    xx = ee.List.sequence(xmin, ee.Number(xmax).subtract(
        ee.Number(dx).multiply(0.9)), dx)
    yy = ee.List.sequence(ymin, ee.Number(ymax).subtract(
        ee.Number(dy).multiply(0.9)), dy)
    ...
    return ee.FeatureCollection(cells).filterBounds(aoi).aggregate_array(
        '.geo').getInfo(), ee.FeatureCollection(cells).filterBounds(aoi)